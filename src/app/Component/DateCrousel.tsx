...
const DateCarousel = ({selectedDate, setSelectedDate}:{selectedDate:any,setSelectedDate:any}) => {
  ...
  useEffect(() => {
    if (selectedDate) {
      const selectedIndex = yearDates.findIndex((date:any) => date.isSame(selectedDate, "day"));
      if (selectedIndex !== -1) {
  ...
  return (
      <div className='flex flex-row items-center justify-between gap-[9px] w-36 overflow-scroll pb-2 sm:pb-0 sm:overflow-hidden md:overflow-visible mx-2 sm:mx-0 my-2'>
        <button
        ...
        {yearDates.slice(activeIndex, activeIndex + (viewPort?yearDates.length:5)).map((date:any, i:number) => {
          ...
          return (
            <button
              ...
              className={`py-[4px] shadow-btn leading-4 flex flex-col justify-center items-center px-[16px] rounded-[7px] text-[14px] ${isSelected ? `bg-[#4f83b8] text-white`:``} font-medium`} 
            >
              <p>{dayName}</p>
